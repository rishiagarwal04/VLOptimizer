...
class OptimizationProvider extends ChangeNotifier {
  ...
  // State
  String _status = 'idle'; // idle, parsing, ready, submitting, processing, completed, error
  String? _fileName;
  ...
  Future<void> _processFile(PlatformFile file) async {
    ...
    if (file.name.endsWith('.json')) {
      ...
      // Excel file: upload to backend for Python parsing (use bytes for web support)
  ...
  /// Submit optimization request (same payload as web's handleOptimize)
  ...
  /// Go back to editing from results
  void goBackToEdit() {
    _status = 'ready';
    _solution = null;
    notifyListeners();
