...
class _VisualizationTabState extends State<VisualizationTab>
  ...
  LatLng _getCenter() {
    ...
    if (allStops.isEmpty) return const LatLng(28.6139, 77.2090); // Delhi default
  ...
  List<Polyline> _buildPolylines() {
    ...
    for (int i = 0; i < routes.length; i++) {
      // Vehicle-level filter from legend
      ...
      // If an employee is selected and is on THIS route, draw the employee segment highlighted
      if (hasEmployeeSelection && isThisRouteForEmployee) {
        // Find pickup and dropoff stop indices for the selected employee
        int? pickupIdx;
        int? dropoffIdx;
        ...
        if (geometry != null && geometry.isNotEmpty) {
          ...
          // Draw highlighted segment between pickup and dropoff
          if (pickupIdx != null && dropoffIdx != null) {
            ...
            // Always use geometry for curved path
      ...
      // If an employee is selected on a DIFFERENT route, dim this route
      if (hasEmployeeSelection && !isThisRouteForEmployee) {
        final geometry = _routeGeometries[i];
        if (geometry != null && geometry.isNotEmpty) {
          polylines.add(Polyline(
      ...
      // No employee selected — normal drawing
      final geometry = _routeGeometries[i];
      if (geometry != null && geometry.isNotEmpty) {
        polylines.add(Polyline(
          points: geometry,
          ...
          child: GestureDetector(
            onTap: () {
              if (_selectedEmployeeId == employeeId) {
                ...
                // Select this employee and highlight their path
                setState(() {
