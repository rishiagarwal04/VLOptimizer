...
class EmployeesTab extends StatelessWidget {
  ...
  Widget _typeBadge(String type, bool isDark, {bool isVehicleType = false}) {
    // For vehicle types (4w, 2w, etc), use simple styling
    if (isVehicleType) {
      return Container(
    ...
    if (lowerType.contains('electric') || lowerType.contains('ev') || lowerType.contains('elec')) {
      ...
      badgeColor = const Color(0xFF10B981); // Green for electric
    } else if (lowerType.contains('petrol') || lowerType.contains('gas')) {
      icon = Icons.local_gas_station;
      badgeColor = const Color(0xFFF59E0B); // Orange for petrol
    } else if (lowerType.contains('diesel')) {
      icon = Icons.local_gas_station;
      badgeColor = const Color(0xFF8B5CF6); // Purple for diesel
    } else if (lowerType.contains('hybrid')) {
      icon = Icons.eco;
      badgeColor = const Color(0xFF06B6D4); // Cyan for hybrid
    }
  ...
  Widget _fuelTypeBadge(String fuelType, bool isDark) {
    // Determine icon and color based on fuel type
    IconData icon;
    Color badgeColor;
    String displayText;
    ...
    if (lowerType.contains('electric') || lowerType.contains('ev') || lowerType.contains('elec')) {
      ...
      icon = Icons.local_gas_station;
      badgeColor = const Color(0xFF6B7280); // Gray for unknown
